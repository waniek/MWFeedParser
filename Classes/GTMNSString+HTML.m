...
//		_GTMDevLog("Unable to allocate buffer or data2");
	...
	for (NSUInteger i = 0; i < length; ++i) 
  {
		...
		if (val || (escapeUnicode && buffer[i] > 127)) 
    {
			if (buffer2Length) 
      {
        CFMutableStringRef cfFinalStrig = (__bridge_retained CFMutableStringRef)finalString;
				CFStringAppendCharacters(cfFinalStrig, 
										 buffer2, 
										 buffer2Length);
				buffer2Length = 0;
			}
		...
		if (length > 3 && length < 11) 
    {
			if ([escapeString characterAtIndex:1] == '#') 
      {
				...
				if (char2 == 'x' || char2 == 'X') 
        {
					...
					unsigned value;
					if ([scanner scanHexInt:&value] &&  value < USHRT_MAX &&
      ...
      else 
      {
        ...
        {
          /* TODO: do it nicer */
					if ([escapeString isEqualToString:[NSString stringWithFormat:@"%s", gAsciiHTMLEscapeMap[i].escapeSequence]]) 
          {
